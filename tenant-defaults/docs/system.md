...
| **`dashboard.json`** (optional) | Declares which dashboard tabs exist for **this** workspace. Omitting the file (or using `"pages": []`) yields **Home + Files** only (multi-user). Prefer **`sections`**: **`todos`**, **`datatable`**, **`funnel_bars`**, **`progress_card`**, **`stat_cards`** (KPI cards), **`grouped_accordion`** (expandable groups), **`metric_datatable`** (headline + table); aliases **`job_pipeline`** / **`week_card`** keep launchpad defaults. Use **`layout`** (**`half`** / **`condensed`** / **`narrow`**) for a half-width column on wider screens. Also: legacy domain templates, **`action_domain`**, or a single **`datatable`** page. Section **`id`** values: lowercase letters, digits, hyphens. See **`dashboard.example.json`** and the host’s **`docs/system.md`**. |
