...
| **`dashboard.json`** (optional) | Declares which dashboard tabs exist for **this** workspace. Omitting the file (or using `"pages": []`) yields **Home + Files** only (multi-user). Templates: **`career`** / **`finance`** / **`business`** (need the matching `*.db` files), **`action_domain`** + **`domain`** (`career` … `family` — open action items from **`brain.db`** only), **`datatable`** (`db` + one **`SELECT`**), **`sections`** (several datatable children on one tab). See **`dashboard.example.json`** and the host’s **`docs/system.md`** (Dashboard manifest). |
